...
lamp_Component = function(param){
		...
		dialog = $( this.add_new_form ).dialog({
			...
			buttons: {
				Add: function() {
					...
					$(self.mapeditor.map).gmap3({
						marker:{
								values:[{
					   				latLng:[form_lat,form_lng],
					    			data:{
					    					type:'lamp',
							    			device_type:'lamp',
							    			network_id:form_lng,
	...
	this.setOptions = function(network_id,device_id){
		self = this;
	...
	this.showInfo = function(marker,context){
		self = this;
		self.parent = component_info_panel = self.mapeditor.componentInfoPanel;
		// self.parent.empty();
		// console.log(marker);
		// console.log(context);
		device_info = context.data;
		...
		$.get('layout/lampinfo1.html',function(data){
			self.element = $('<div id="lamp-editor"></div>').appendTo(self.parent);
			data = data.replace("{type}", device_info.device_type);
			data = data.replace("{latitude}", marker.position.lat());
			data = data.replace("{longitude}", marker.position.lng());
	...
	this.renderTool = function(lamp_component){
		add_lamp_btn = $('<input type="button" value="Add Lamp">').appendTo(this.mapeditor.componentToolPanel).addClass('atk-button-small');
		add_lamp_btn.on('click',function(){
			lamp_component.addLamp();
		});
	}
